-- Ensure the agencies table exists with the correct structure
-- This script is idempotent and can be run multiple times

-- Create the agencies table if it doesn't exist
...
SELECT 
  ...
  4.8,
  245
WHERE NOT EXISTS (SELECT 1 FROM agencies LIMIT 1);

-- Insert more sample agencies if needed
INSERT INTO agencies (name, description, logo, website, phone, email, address, islands, services, rating, reviews_count)
SELECT 
  'Island Wheels',
  ...
  4.5,
  189
WHERE EXISTS (SELECT 1 FROM agencies LIMIT 1) AND NOT EXISTS (SELECT 1 FROM agencies WHERE name = 'Island Wheels');
...
SELECT 
  ...
  4.7,
  312
WHERE EXISTS (SELECT 1 FROM agencies LIMIT 1) AND NOT EXISTS (SELECT 1 FROM agencies WHERE name = 'Blue Aegean Rentals');
...
SELECT 
  ...
  4.6,
  278
WHERE EXISTS (SELECT 1 FROM agencies LIMIT 1) AND NOT EXISTS (SELECT 1 FROM agencies WHERE name = 'Greek Island Motors');
...
SELECT 
  ...
  4.3,
  156
WHERE EXISTS (SELECT 1 FROM agencies LIMIT 1) AND NOT EXISTS (SELECT 1 FROM agencies WHERE name = 'Cyclades Auto');
